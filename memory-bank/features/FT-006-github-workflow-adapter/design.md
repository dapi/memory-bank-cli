...
- `SOL-01` Add the explicit opt-in boundary `memory-bank-cli github init|update`; do not alter existing generic `init` or `update` semantics.
...
- `INV-01` No command outside `memory-bank-cli github` installs adapter assets.
