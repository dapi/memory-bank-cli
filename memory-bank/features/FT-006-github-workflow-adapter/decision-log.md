...
| --- | --- | --- | --- | --- | --- |
| `DEC-01` | accepted | Use the explicit `mb-cli github init|update` boundary with embedded assets and marker-owned blocks. | Issue #40 permits an opt-in command/config, requires no core dependency, specifies managed-marker/manifest ownership and asks that existing templates remain user-owned. Existing generic `init`/`update` already have a separate Memory Bank lock contract. | FPF B.5: a separate subcommand is the smallest hypothesis preserving the generic contract. Tests deductively verify opt-in invocation, preservation, conflict and dry-run predictions. | [design.md](design.md) `SOL-01`–`SOL-03` |
