...
| `DEC-03` | accepted | Use a validation workflow plus a tag-driven workflow that repeats validation before GoReleaser GitHub Release publication. | Issue #3 requires tests, static analysis and release builds before publication; `.goreleaser.yml` already defines a single GitHub release build. | Assurance reasoning: GitHub Release publication is a high-impact claim, so the evidence-producing validation must be a prerequisite of that same release execution rather than an assumed earlier event. | [design.md](design.md) `SOL-01`, `SOL-02`, `INV-01` |
...
| `DEC-09` | accepted | Treat a pushed semantic tag as an immutable public Go module version; never repoint it after a tag-triggered validation failure. | `CON-02` requires consumers to install the tag through `go install`; the release workflow runs validation after the tag trigger; Go proxies may cache the tagged module version. | Boundary and evidence reasoning separates the Go-module publication caused by the tag from later GitHub Release publication. Once the first boundary has occurred, rollback cannot restore the prior state; correction therefore requires a distinct, forward-only semantic version and renewed human authorization. | [brief.md](brief.md) `CON-01`, `REQ-01`, `EC-01`; [design.md](design.md) `CTR-01`, `FM-01`, `RB-02`; [implementation-plan.md](implementation-plan.md) `ER-01`, `STOP-01`–`STOP-04` |
