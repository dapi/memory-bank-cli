...
| --- | --- | --- |
| `internal/ownership` source reader/verifier | validate committed source root and translate source-relative payload paths | Git/filesystem `memory-bank-template/*` → in-process payload map keyed by `memory-bank/*` |
...
| Quality / evolution concerns | covered | `TRD-01`–`TRD-03`, `RB-01`–`RB-03` | decision log | Strict target source root, compatibility regressions and release-before-rename ordering bound migration risk. |

## Selected Solution

- `SOL-01` Introduce distinct internal source-root and downstream-root concepts. Source inspection, dirty/ignored checks and pinned Git-tree reads use `memory-bank-template/`; ownership classification, decisions, locks, mutations and init/update-generated downstream agent links continue to use `memory-bank/`.
- `SOL-02` Translate each accepted source entry by stripping exactly the `memory-bank-template/` prefix and prefixing the unchanged relative suffix with `memory-bank/` before the payload map reaches the planner. Apply the same mapping to Git-backed production reads and filesystem-backed test reads.
...
| `ALT-02` | Support both source roots with discovery/fallback | Issue #63 requires no upstream duplicate and issue #14 excludes indefinite dual-path support; fallback adds ambiguity and can inspect the wrong tree. |
...
| `TRD-02` | Reject the old upstream payload root instead of providing fallback | Target state is deterministic and matches the coordinated no-duplicate rename. | The new release cannot read pre-rename source checkouts; existing released CLI remains available for those checkouts during coordination. |
...
- `SD-01` Internal terminology is `source payload root` = `memory-bank-template/`, `downstream payload root` = `memory-bank/`, and `payload-relative path` = the suffix below either root.
- `SD-02` Source root matching is exact and case-sensitive, consistent with Git tree paths; no alternative spelling or fallback root is accepted.
...
| --- | --- | --- | --- |
| `CTR-01` | pinned Git/filesystem source `memory-bank-template/<suffix>` → ownership payload `memory-bank/<suffix>` | CLI initiates synchronous local read; source checkout provides committed blobs | exact prefix translation only; regular files and supported Git modes retain contents/modes; missing/dirty/ignored/unsupported source fails before planning. |
...
- `FM-01` The pinned commit lacks `memory-bank-template/` or contains only the old source root. Fail source verification before a plan is produced.
- `FM-02` Source status/tree checks accidentally examine `memory-bank/` or the whole checkout but miss ignored/unsupported content under the new root. Targeted fixtures must fail the change until both checks use the source root.
...
| State / transition completeness | yes | Release-before-template-merge ordering has safe/unsafe states. | Walk through `RB-01`–`RB-03` and issue coordination gates. | Only a validated approved release can unblock the rename; failure retains the old template state. |
...
| Migration / evolution safety | yes | CLI release must precede removal of the old source root. | Compatibility matrix and rollout walk-through. | New CLI/new source is supported; new CLI/old source is deliberately rejected; template rename remains blocked until `RB-02` evidence. |
...
| `REQ-06` | `SOL-05`, `SD-04` | `INV-05` | `FM-05`, `RB-02`, `RB-03` |
