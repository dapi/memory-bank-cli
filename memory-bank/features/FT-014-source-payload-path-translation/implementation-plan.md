...
| --- | --- | --- | --- |
| `internal/ownership/source.go`: `verifySourceCheckout` | Verifies checkout root/HEAD/clean status, then checks ignored content with pathspec `memory-bank`. | `TestSourceRefMustMatchCleanGitCheckout`; hidden-worktree and CRLF tests prove pinned-object behavior. | Replace only the payload pathspec/error terminology with the source-root contract; retain checkout/ref and double-verification safety. |
| `internal/ownership/source.go`: `verifySourcePayload` | Runs `git ls-tree ... -- memory-bank`, accepts blob modes `100644`/`100755`, rejects empty/unsupported tree. | `TestPinnedSourceExecutableModeIsInstalled`. | Select `memory-bank-template` and preserve all type/mode rejection behavior. |
...
| Unresolved | none | No discovery spike is required before `STEP-01`; any contradictory runtime evidence activates `STOP-01`. |

## Test Strategy

| Test surface | Canonical refs | Existing coverage | Planned automated coverage | Required local suites / commands | Required CI suites / jobs | Manual-only gap / justification | Manual-only approval ref |
| --- | --- | --- | --- | --- | --- | --- | --- |
| source verification/read | `REQ-01`, `REQ-02`, `SC-01`, `NEG-01`, `NEG-02`, `CTR-01` | `TestSourceRefMustMatchCleanGitCheckout`, hidden-worktree, CRLF and executable-mode cases use old source root | migrate source setup/pathspecs to new root; add old-root-only/missing-new-root rejection and explicit source→downstream key assertions | `go test ./internal/ownership -run 'Test(Source|Pinned|CleanCheckout)'`; then full package | repository Validate job | none | none |
...
| release/handoff | `REQ-06`, `SC-06`, `NEG-04`, `INV-05`, `RB-02`, `RB-03` | protected manual release workflow | exact-commit validation, approval, clean install/version and issue #63 handoff | `CHK-06`, `CHK-07` | Validate + Publish release jobs | public publication and external template CI handoff require human/repository action | `AG-01` |
...
| `SD-04`, `INV-05`, `FM-05`, `RB-02`, `RB-03` | `design.md` | release workflow run, immutable tag and issue #63 handoff | `STEP-06`, `STEP-07` | `CHK-06`, `CHK-07` | `EVID-06`, `EVID-07` |

## Workstreams

| Workstream | Implements | Result | Owner | Dependencies |
| --- | --- | --- | --- | --- |
| `WS-1` | `REQ-01`–`REQ-03`, `SOL-01`, `SOL-02` | strict source reader/translation with unchanged downstream ownership behavior | either | `PRE-01` |
...
| `WS-4` | `REQ-06`, `RB-02`, `RB-03` | approved compatible release and template-side handoff | human/either | `WS-3`, `PRE-02`, `PRE-03` |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `STEP-01` | agent/either | `REQ-01`, `REQ-02`, `SOL-01`, `CTR-01` | define exact source/downstream root constants and move all verification selectors to the source root | `verifySourceCheckout`, `verifySourcePayload`, source Git fixtures | verified new-root source boundary with unchanged pinning/mode rules | `CHK-01` | `EVID-01` | run targeted source tests including old-root-only rejection | `PRE-01` | none | old-root fallback appears necessary or issue #63 contract changed |
| `STEP-02` | agent/either | `REQ-02`, `REQ-03`, `SOL-02`, `CTR-02` | translate once in both source readers and preserve planner/lock/transaction behavior | `readSource`, `readGitSource`, `buildPlan` boundary, ownership fixtures/tests | congruent downstream-keyed payload from both reader branches | `CHK-02` | `EVID-02` | targeted init/update/transaction tests; inspect decision paths and decoded lock keys | `STEP-01` | none | readers disagree, any source-prefixed key reaches planner, or safety verdict changes without owner update |
...
| --- | --- | --- | --- |
| `CP-01` | `STEP-01`, `STEP-02`, `CTR-01`, `CTR-02` | new-root source passes into an exclusively downstream-keyed planner without safety regression | `EVID-01`, `EVID-02` |
...
- `CHK-01`–`CHK-07` have concrete `EVID-01`–`EVID-07` carriers.
