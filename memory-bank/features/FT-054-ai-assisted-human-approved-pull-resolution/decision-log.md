...
| `DEC-02` | accepted | Treat every two-sided `adapted` path in the authorized affected-path set as unresolved until a human records one of the issue-required actions; a mechanical merge records verified base bytes from a lock-bound historical-base sidecar state, local/upstream inputs, the `mbc-diff3-lines-v1` algorithm version, result bytes and mode, and is recomputed at apply. The sidecar defines absent or unavailable base state only when its fresh generation and canonical digest match an independently verifiable protected receipt; `.lock` retains its required last-present digest/mode, and the sidecar path is reserved from payload planning. | Issue #54 names the required actions, prohibits silent adapted decisions and permits a mechanical merge only when its exact result is encoded in the reviewed plan. Current `--ask` already separates answer collection from mutation. Existing locks retain identities/digests and modes only, so a legacy, missing, stale, replayed or unreceipted sidecar entry without retained base state cannot mechanically merge. | FPF strict boundary and evidence-chain reasoning separates proposal (agent/AI) from authorization (human) and deterministic execution (CLI). A result digest alone can describe arbitrary bytes, and a lock identity alone cannot distinguish intentional present/absent transitions with the same legacy serialization; independently attesting each non-reusable sidecar generation prevents historical replay while preserving the older-readable `.lock` format. The normative merge algorithm proves the limited mechanical-merge claim. Explicit first adoption rejects pre-existing reserved-path content, and unavailable state permits full-scope authorized non-merge recovery without inferring historical records. | [design.md](design.md) `SOL-03`, `CTR-02`, `INV-03`, `INV-07` |
