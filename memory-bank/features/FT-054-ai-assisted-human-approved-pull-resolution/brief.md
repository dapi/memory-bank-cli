...
- `REQ-03` Apply a resolution plan only after validating its lock, source identity, local payload, template payload and lock-bound historical-state sidecar under a cooperating-writer serialization boundary; reserve `memory-bank/.lock-history-v1` exclusively for that sidecar, reject downstream/template payload collisions before mutation, and perform final revalidation and apply all accepted actions, `.lock` and that sidecar atomically without changing the existing `.lock` wire format.
...
- `SC-04i` A downstream user file, template payload file, malformed/stale sidecar, or plan action at the reserved `memory-bank/.lock-history-v1` path is rejected without mutation; no payload planning, scope selector, implicit overwrite or automatic migration can claim that path.
...
- `NEG-04` Reject an existing downstream or template payload file at `memory-bank/.lock-history-v1`, a malformed/stale sidecar there, or a plan/scope that includes that reserved control path.
