...
- `SC-04i` A downstream user file, template payload file, malformed control-state record, or plan action at a reserved internal control path is rejected without mutation; no payload planning, scope selector, implicit overwrite or automatic migration can claim that path. A well-formed control-state record with a stale lock binding is not a payload collision and makes mechanical merge unavailable. It may be atomically replaced only by a full-scope validated clean-baseline/update that uses no historical state, has only deterministic clean actions that establish or remove current upstream bases, and can create every required retained absent state. That transaction discards every stale record and snapshot; it retains only newly derived post-state records, while unrelated current lock entries receive no recovered record and remain historical-state unavailable. A bounded-scope plan or any other action with a stale sidecar is rejected without mutation.
