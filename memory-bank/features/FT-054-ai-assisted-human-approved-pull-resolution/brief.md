...
- `SC-04c` For a managed path with no local drift, apply retains `managed` ownership and atomically follows the upstream state: when upstream is present and changed, it writes the recorded upstream bytes and executable mode; when upstream is absent, it deletes the local path and preserves lock compatibility and an absent upstream state for later resolution. On a later no-local-drift pull from that absent state, an upstream reappearance deterministically writes the upstream bytes and executable mode and retains `managed` ownership.
...
- `SC-04h` An oversized serialized plan carrier is rejected before decode, and carrier, action-count, nesting, path, identity and metadata boundary cases are rejected without mutation. Bounded historical-state handling produces deterministic, untruncated retained or unavailable outcomes on identical fixtures; the selected design defines its representation, accounting and allocation rules.
...
| `CHK-03` | `EC-03`, `SC-03`–`SC-04i`, `NEG-02`, `NEG-04` | Run hermetic E2E for full and explicit-scope affected-path coverage; the required deterministic and human-selected outcomes; compatible historical-state availability and unavailability; collision-safe control-state handling; reviewed-merge eligibility and result verification; bounded-resource behavior; and rollback compatibility. The selected design owns the representation, algorithms, allocation rules and exact resource thresholds exercised by this check. | Omitted or unresolved selected affected paths fail; scoped apply leaves unselected paths unchanged; each required safe action produces its specified content, mode, ownership and compatibility-preserving state; unavailable historical state cannot enable a merge; control state cannot be treated as payload; only an explicit allowed two-sided action meeting verified merge preconditions yields its exact reviewed result; and bounded inputs have safe, untruncated outcomes while permitted non-merge actions remain available. | `artifacts/ft-054/verify/chk-03/` |
