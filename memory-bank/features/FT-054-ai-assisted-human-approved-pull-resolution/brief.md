...
| `implementation-plan.md` | deferred | Execution will cross CLI, ownership, lock, transaction, documentation and E2E surfaces, but `BD-01` leaves an upstream security-boundary decision unresolved. Create the plan only after that decision is incorporated and reviewed. | none until `BD-01` is resolved |
...
- `SC-04i` A downstream user file, template payload file, malformed control-state record, or plan action at a reserved internal control path is rejected without mutation; no payload planning, scope selector, implicit overwrite or automatic migration can claim that path. First adoption is explicit and succeeds only after the CLI observes that both its protected control-state provenance record and the reserved path are absent; any pre-existing object at that path, even one that is well formed and lock-bound, is rejected as user content. A sidecar is authoritative only when an independently verifiable, monotonic provenance receipt binds its fresh generation and canonical digest to the current exact lock identity; a stale, replayed, missing or mismatched receipt makes mechanical merge unavailable. Such unavailable state may be atomically recovered only by a validated full-scope plan that recomputes without historical state and contains no unresolved or reviewed-merge action. Its resolved actions may be deterministic non-merge actions or authorized allowed non-merge selections, including `keep-local` and `take-upstream`. The transaction discards every unavailable record and snapshot, retains only newly derived post-state records under a new generation, and finalizes the matching provenance receipt after the payload/lock/control-state commit; unrelated current lock entries receive no recovered record and remain historical-state unavailable. A bounded-scope, unresolved or reviewed-merge action with unavailable sidecar state is rejected without mutation.
...
- `NEG-04` Reject an existing downstream or template payload file at a reserved internal control path, including a well-formed would-be sidecar during first adoption, a malformed control-state record, or a plan/scope that includes that path. A stale, replayed, missing or mismatched provenance receipt remains unavailable for merge rather than being treated as a payload collision or authoritative state.
...
| `CHK-03` | `EC-03`, `SC-03`–`SC-04i`, `NEG-02`, `NEG-04` | Run hermetic E2E for full and explicit-scope affected-path coverage and complete decision-inventory regeneration; the required deterministic and human-selected outcomes; managed present-base local drift with unchanged, changed and absent upstream; compatible historical-state availability and unavailability; durable absent-state retention and rejection when it cannot be retained; managed-absent local recreation with upstream absent and with upstream reappearing; independent sidecar-generation/receipt verification, replay rejection and first-adoption collision rejection; full-scope unavailable-sidecar recovery using authorized non-merge selections, rejected bounded/unresolved/merge recovery and stale-record disposal; collision-safe control-state handling; reviewed-merge eligibility and result verification; bounded-resource behavior; and rollback compatibility. The selected design owns the representation, algorithms, allocation rules and exact resource thresholds exercised by this check. | Omitted or unresolved selected affected paths fail; a bounded plan exposes authenticated decisions for every omitted path and scoped apply leaves those paths unchanged; each required safe action produces its specified content, mode, ownership and compatibility-preserving state; unavailable historical state cannot enable a merge; a replayed or unproven sidecar is never authoritative; an eligible full-scope recovery preserves authorized non-merge behavior while retaining only newly derived state; control state cannot be treated as payload; only an explicit allowed two-sided action meeting verified merge preconditions yields its exact reviewed result; and bounded inputs have safe, untruncated outcomes while permitted non-merge actions remain available except where durable absent-state retention is required. | `artifacts/ft-054/verify/chk-03/` |
