...
- `EC-02` A human-authorized plan containing only deterministic actions applies atomically to its matching serialized pre-state and exactly covers either the full affected-path set or its declared authorized scope. Re-executing it against a changed post-state is rejected as stale; executing it on separate identical pre-state fixtures produces the same result.
