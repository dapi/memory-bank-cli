...
| `CHK-03` | `EC-03`, `SC-03`–`SC-04i`, `NEG-02`, `NEG-04` | Run hermetic E2E for full and explicit-scope affected-path coverage; the required deterministic and human-selected outcomes; compatible historical-state availability and unavailability; durable absent-state retention and rejection when it cannot be retained; managed-absent local recreation with upstream absent and with upstream reappearing; collision-safe control-state handling; reviewed-merge eligibility and result verification; bounded-resource behavior; and rollback compatibility. The selected design owns the representation, algorithms, allocation rules and exact resource thresholds exercised by this check. | Omitted or unresolved selected affected paths fail; scoped apply leaves unselected paths unchanged; each required safe action produces its specified content, mode, ownership and compatibility-preserving state; unavailable historical state cannot enable a merge; an accepted absent upstream state remains available for subsequent resolution; control state cannot be treated as payload; only an explicit allowed two-sided action meeting verified merge preconditions yields its exact reviewed result; and bounded inputs have safe, untruncated outcomes while permitted non-merge actions remain available except where durable absent-state retention is required. | `artifacts/ft-054/verify/chk-03/` |
