...
`DEC-04` blocks execution: a human must approve the attestation/signature format, canonicalization procedure, trust-store configuration, key lifecycle and reviewer signing workflow. Exact flag spelling/field names may be selected during implementation only after that authorization contract is approved.
...
| `WS-02` | `SOL-02`–`SOL-04`, `CTR-02`, `INV-02`–`INV-06`, `FM-*` | either | `WS-01` |
...
| `SOL-03`–`SOL-04`, `SD-02`, `CTR-02`, `INV-02`–`INV-07`, `FM-01`–`FM-02` | retained historical-base snapshot, apply validation, mechanical-merge derivation/recomputation and adapted/deterministic action resolution | `STEP-02` | `CHK-02`–`CHK-04` / `EVID-02`–`EVID-04` |
...
| `STEP-02` | either | `REQ-03`–`REQ-05`, `SOL-03`–`SOL-04`, `CTR-02`, `INV-*` | Add validated apply and action resolution, including atomic retention and digest verification of historical-base snapshots, an exclusive update guard, transaction-scoped final revalidation, deterministic three-way merge recomputation, reviewed result/mode, managed no-drift update, and deterministic keep-and-detach that preserves bytes/mode while removing its lock entry. Mark mechanical merge unavailable for legacy entries without a verified snapshot; never synthesize one from a digest. | `internal/ownership`, CLI tests/E2E | `CHK-02`, `CHK-03` | `EVID-02`, `EVID-03` | `STEP-01`, `PRE-02` | human decision and valid authorization exist before apply; no runtime prompt required | an action, mechanical merge or final pre-commit state cannot be validated before staging |
