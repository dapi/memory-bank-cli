...
| `internal/ownership/lock.go`, `types.go` | Lock decoding is strict and lock entries hold ownership, digests and modes. | Bind plan identity checks to the same validated state. |
...
| `PRE-02` | No apply is attempted until strict plan validation and all required human decisions succeed. | `INV-02`, `INV-03`; `CHK-03`–`CHK-04`. |

## Workstreams

| Workstream | Covers | Owner | Depends on |
| --- | --- | --- | --- |
| `WS-01` | `SOL-01`, `CTR-01`, `INV-01` | either | `PRE-01` |
| `WS-02` | `SOL-02`, `CTR-02`, `INV-02`–`INV-04`, `FM-*` | either | `WS-01` |
...
| --- | --- | --- | --- |
| `SOL-01`, `SD-01`, `CTR-01`, `INV-01` | ownership plan model/strict JSON codec; CLI planning flag/output | `STEP-01` | `CHK-01` / `EVID-01` |
| `SOL-02`, `SD-02`, `CTR-02`, `INV-02`–`INV-04`, `FM-01`–`FM-02` | apply validation and adapted/deterministic action resolution | `STEP-02` | `CHK-02`–`CHK-04` / `EVID-02`–`EVID-04` |
| `SOL-03`, `FM-03`, `RB-01` | reuse atomic transaction; help/README/use case; regression suites | `STEP-03`–`STEP-04` | `CHK-04`–`CHK-05` / `EVID-04`–`EVID-05` |

## Steps

| Step ID | Actor | Implements | Goal | Touchpoints | Verifies | Evidence IDs | Blocked by | Needs approval | Escalate if |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `STEP-01` | either | `REQ-01`–`REQ-02`, `SOL-01`, `CTR-01` | Add versioned strict plan model and read-only planning output with required per-path context. | `internal/ownership`, `internal/cli`, focused tests | `CHK-01` | `EVID-01` | `PRE-01` | none | representation cannot bind all required review identities |
| `STEP-02` | either | `REQ-03`–`REQ-05`, `SOL-02`, `CTR-02`, `INV-*` | Add validated apply and action resolution, including encoded merge bytes/mode and deterministic keep-and-detach. | `internal/ownership`, CLI tests/E2E | `CHK-02`, `CHK-03` | `EVID-02`, `EVID-03` | `STEP-01`, `PRE-02` | human decision exists only in plan data; no runtime prompt required | an action cannot be validated before staging |
| `STEP-03` | either | `REQ-03`, `REQ-07`, `SOL-03`, `FM-*` | Route valid results through existing atomic transaction and add stale/tamper/injected-failure E2E. | ownership transaction and regression tests | `CHK-04` | `EVID-04` | `STEP-02` | none | payload and lock cannot be proved unchanged after failure |
...
| --- | --- | --- | --- |
| `CP-01` | `STEP-01` | Plan is read-only, strict and contains every required identity/context. | `EVID-01` |
| `CP-02` | `STEP-02` | Every adapted two-sided path requires an explicit valid action and exact recorded output. | `EVID-02`, `EVID-03` |
...
Automated coverage: plan codec/read-only CLI output (`CHK-01`); deterministic safe apply (`CHK-02`); all adapted actions, unresolved rejection and modes (`CHK-03`); stale/tamper/path and transaction interruption (`CHK-04`); help/docs plus applicable Go test/vet (`CHK-05`). These are hermetic local fixtures and are also the required CI evidence under the selected standard profile. No manual-only acceptance gap is accepted: human approval is represented as deterministic plan data in tests, while real review remains a human operational act.
