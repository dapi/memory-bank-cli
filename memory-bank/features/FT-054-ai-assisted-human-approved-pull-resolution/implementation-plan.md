...
| `internal/ownership/lock.go`, `types.go` | Lock decoding is strict and lock entries hold ownership, digests and modes. | Preserve the `.lock` wire format; add an atomically written, lock-bound historical-base sidecar for future mechanical merges. Legacy or missing sidecar state cannot use mechanical merge. |
...
| `CP-02` | `STEP-02` | The plan exactly matches the regenerated deterministic non-decision review context and covers the recomputed full or explicitly scoped affected-path set; every adapted two-sided path in scope requires an explicit valid action; present/absent transitions have the specified tombstone outcome; every mechanical merge is reproducible from validated inputs and has the exact recorded output; strict old-lock decoding survives new sidecar creation; and every `SOL-05` carrier, structural, retention and merge resource bound has its specified safe outcome. | `EVID-02`, `EVID-03`, `EVID-04` |
