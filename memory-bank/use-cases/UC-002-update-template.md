...
1. The actor runs `memory-bank-cli pull`, optionally first with `--dry-run`. During the coordinated payload rename, the pinned source contains exactly one recognized root—legacy `memory-bank/`, legacy `memory-bank-template/`, or target `template/memory-bank/`—while the existing locked destination stays `memory-bank/`.
