...
func TestProfileDetectionUsesTemplatePayloadRoot(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			report, err := Run(Options{RepoRoot: fixture(t, test.fixture), ScopeRoot: "memory-bank", AgentFile: "AGENTS.md", Profile: test.profile, MaxDepth: 3})
			if err != nil {
				t.Fatal(err)
			}
			if report.Profile != test.want {
				t.Fatalf("profile = %q, want %q", report.Profile, test.want)
			}
		})
	}
}
...
func TestGovernanceCycleAndLifecycleFindingsHaveStableCodes(t *testing.T) {
	...
	write(".github/workflows/ci.yml", "run: memory-bank-cli doctor\n")
...
func TestWorkflowRunsDoctorOnlyForExecutableRunCommands(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "direct command", workflow: "jobs:\n  check:\n    steps:\n      - run: memory-bank-cli doctor --profile downstream\n", want: true},
		{name: "multiline after separator", workflow: "jobs:\n  check:\n    steps:\n      - run: |\n          make prepare && memory-bank-cli doctor\n", want: true},
	} {
		t.Run(test.name, func(t *testing.T) {
