...
func TestSourceFormatRejectionIsNonMutating(t *testing.T) {
	cases := []struct{ name, declaration, marker, want string }{
		...
		{"missing capability", `{"schema_version":1,"payload_format":"legacy/v1"}`, "", "requires exactly capability"},
		{"unsupported capability", `{"schema_version":1,"payload_format":"legacy/v1","capabilities":["legacy/v1","components/v1"]}`, "", "requires exactly capability"},
		{"CLI-only capability", `{"schema_version":1,"payload_format":"legacy/v1","capabilities":["legacy/v1","source-format/v1"]}`, "", "requires exactly capability"},
		{"duplicate capability", `{"schema_version":1,"payload_format":"legacy/v1","capabilities":["legacy/v1","legacy/v1"]}`, "", "requires exactly capability"},
