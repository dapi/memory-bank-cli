...
scenario_15() { setup_case E2E-15; init_v1; local lock_before; lock_before="$(cat "$(lock_file)")"; rm "$(file memory-bank/dna/managed.md)"; ln -s "$case_root/outside" "$(file memory-bank/dna/managed.md)"; checkout_v2; expect_fail update_v2; test -L "$(file memory-bank/dna/managed.md)"; test "$(readlink "$(file memory-bank/dna/managed.md)")" = "$case_root/outside"; test "$(cat "$(lock_file)")" = "$lock_before"; assert_absent "$(file memory-bank/dna/created.md)"; }
